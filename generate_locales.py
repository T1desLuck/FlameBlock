...
TRANSLATIONS = {
    ...
    "extDescription": {
        "ru": "Продвинутый блокировщик рекламы и трекеров: сетевые списки, косметическая фильтрация, эвристики и MAIN-world скриптлет против попандеров и анти-adblock скриптов. Работает полностью локально, ничего не отправляет на сервер.",
        "en": "Advanced ad and tracker blocker: network filter lists, cosmetic filtering, heuristics, and a MAIN-world scriptlet against pop-unders and anti-adblock scripts. Works entirely locally — nothing is ever sent to a server.",
        "zh_CN": "先进的广告与跟踪器拦截器:网络过滤列表、外观过滤、启发式检测,以及针对弹窗广告和反广告拦截脚本的 MAIN-world 脚本。完全本地运行,不会向任何服务器发送数据。",
        "es": "Bloqueador avanzado de anuncios y rastreadores: listas de filtrado de red, filtrado cosmético, heurísticas y un scriptlet en el contexto MAIN contra popunders y scripts anti-adblock. Funciona totalmente en local, no envía nada a ningún servidor.",
        "hi": "विज्ञापन और ट्रैकर के लिए एडवांस्ड ब्लॉकर: नेटवर्क फ़िल्टर सूचियाँ, कॉस्मेटिक फ़िल्टरिंग, हेयुरिस्टिक्स और पॉप-अंडर व एंटी-एडब्लॉक स्क्रिप्ट के खिलाफ़ MAIN-world स्क्रिप्टलेट। पूरी तरह से लोकल रूप से काम करता है, कोई डेटा सर्वर पर नहीं भेजा जाता।",
        "ar": "حاجب إعلانات ومتتبعات متقدم: قوائم فلترة عبر الشبكة، وفلترة تجميلية، وأساليب استدلالية، وسكريبت مصغّر يعمل في سياق الصفحة (MAIN world) لمواجهة النوافذ المنبثقة الخلفية وسكريبتات كشف حاجبات الإعلانات. يعمل بالكامل محليًا، ولا يُرسل أي بيانات إلى أي خادم.",
        "pt_BR": "Bloqueador avançado de anúncios e rastreadores: listas de filtragem de rede, filtragem cosmética, heurísticas e um scriptlet no contexto MAIN contra popunders e scripts anti-adblock. Funciona totalmente localmente, nada é enviado a servidor algum.",
        "ja": "広告とトラッカーを高度にブロック: ネットワークフィルターリスト、コスメティックフィルタリング、ヒューリスティック検知、そしてポップアンダーや広告ブロック検出スクリプトに対抗するMAIN-worldスクリプトレットを搭載。完全にローカルで動作し、サーバーには一切データを送信しません。",
        "ko": "고급 광고 및 트래커 차단기: 네트워크 필터 목록, 코스메틱 필터링, 휴리스틱, 팝언더 및 광고 차단 감지 스크립트에 대응하는 MAIN-world 스크립틀릿까지 지원합니다. 전적으로 로컬에서 동작하며 서버로 어떤 데이터도 전송하지 않습니다.",
        "de": "Fortschrittlicher Werbe- und Tracker-Blocker: Netzwerk-Filterlisten, kosmetische Filterung, Heuristiken und ein MAIN-World-Scriptlet gegen Popunder und Anti-Adblock-Skripte. Funktioniert vollständig lokal, es werden keinerlei Daten an einen Server gesendet.",
        "fr": "Bloqueur avancé de publicités et de traqueurs : listes de filtrage réseau, filtrage cosmétique, heuristiques et un scriptlet exécuté dans le contexte MAIN contre les popunders et les scripts anti-adblock. Fonctionne entièrement en local, aucune donnée n'est jamais envoyée à un serveur.",
        "id": "Pemblokir iklan dan pelacak tingkat lanjut: daftar filter jaringan, pemfilteran kosmetik, heuristik, dan scriptlet MAIN-world untuk melawan popunder dan skrip anti-adblock. Bekerja sepenuhnya secara lokal, tidak ada data yang dikirim ke server mana pun.",
    },
    "pause_all_title": {
